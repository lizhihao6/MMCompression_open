...
class _ActNorm(nn.Module):
    ...
    def __init__(self, num_features, scale=1.0):
        ...
        # use a parameter, so that it won't init params when load state dict.
        self.register_parameter(name='init', param=torch.nn.Parameter(torch.BoolTensor([False]), requires_grad=False))
...
class Block(nn.Module):
    def __init__(self, in_channels, out_channels, hidden_channels):
        ...
        self.res3 = ResBlock(hidden_channels)
...
class MaskBlock(nn.Module):
    def __init__(self, in_channels, out_channels, hidden_channels):
        super().__init__()
        self.res = Block(in_channels, out_channels, hidden_channels)
        self.stem = nn.Sequential(
            Conv2d(in_channels, hidden_channels, kernel_size=(1, 1)),
            nn.ReLU(inplace=False)
        )
        self.non_local = Non_local_Block(hidden_channels, hidden_channels // 2)
        self.res1 = ResBlock(hidden_channels)
        self.res2 = ResBlock(hidden_channels)
        self.res3 = ResBlock(hidden_channels)
        self.root = Conv2dZeros(hidden_channels, out_channels, kernel_size=(1, 1))
    ...
    def forward(self, z):
        mask = self.stem(z)
        mask = self.non_local(mask)
        mask = self.res1(mask)
        mask = self.res2(mask)
        mask = self.res3(mask)
        mask = self.root(mask)
        mask = torch.sigmoid(mask)
        return mask * self.res(z)
