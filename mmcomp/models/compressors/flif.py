...
class FLIFCompressor(BaseCompressor):
    ...
    def _comp(self, img):
        ...
        flif_file = self._get_tmp_file('flif')
        if img.shape[2] == 4:  # raw
            ...
            os.system(f'{self.flif} -e {png_file} {flif_file} > /dev/null 2>&1')
            bits = float(os.path.getsize(flif_file)) * 8
